...
var globber = function (glob, syntax, dump, custom_fn, err_handler) {
    var postcssBefore = [
    ...
    registerTask: function () {
        gulp.task('css', function () {
            ...
            .on('end', function (err) {
                ...
                utils.dumpSettings(dump);
            });
            gulp.src(cascade.path + cascade.css.templateFilePath) //
            .pipe(postcss(after_)) //
            .pipe(gulp.dest(settings.path + '/' + cascade.assetsDirName)).on('end', function (err) {
                if (err) {
