...
gulp.task('js', function () {
    ...
    var glob = [settings.path + '/**/' + cascade_settings.js.fileName, '!' + settings.path + '/{' + cascade.assetsDirName + ',' + cascade.assetsDirName + '/**}'];
    var success = true;
    return gulp.src(glob).pipe(plumber({
        ...
        indentSize: 4
    })).pipe(gulp.dest(settings.path + '/' + cascade.buildDir)).on('end', function (err) {
        if (success === true) {
